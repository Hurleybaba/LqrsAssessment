...
export class WalletService {
  ...
  static async transfer(senderId: string, receiverEmail: string, amount: number) {
    ...
    return db.transaction(async (trx) => {
      // 1. Lock Sender
      const senderWallet = await walletRepo.findByUserIdForUpdate(senderId, trx);
      if (!senderWallet) throw new AppError("Sender wallet not found", 404);
      if (senderWallet.balance < amount) throw new AppError("Insufficient funds", 400);
      ...
      // 3. Lock Receiver
      const receiverWallet = await walletRepo.findByUserIdForUpdate(receiver.id, trx);
      if (!receiverWallet) throw new AppError("Receiver wallet not found", 404);
      if (senderWallet.id === receiverWallet.id) throw new AppError("Self-transfer denied", 400);
